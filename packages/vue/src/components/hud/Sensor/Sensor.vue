...
/** 从 Figma 精确导出的 Sensor 图标 path（与 React 版同源） */
const SENSOR_PATHS = [
...
import styles from '@react/components/hud/Sensor/sensor.module.less'

const props = withDefaults(defineProps<SensorProps>(), { active: true, plus: false, size: 50 })
const color = computed(() => (props.active ? '#9DECFD' : '#658D95'))
</script>

<template>
  <div :class="[styles.container, { [styles.active]: props.active }]" :style="{ width: props.size + 'px', height: props.size + 'px' }">
    <svg aria-hidden="true" viewBox="0 0 50 50" fill="none" :class="styles.bg">
      <circle cx="25" cy="25" r="25" fill="black" fill-opacity="0.8" />
    </svg>
    <svg aria-hidden="true" viewBox="0 0 34 37" fill="none" :class="styles.icon">
      <path v-for="(d, i) in SENSOR_PATHS" :key="i" :d="d" :fill="color" />
    </svg>
