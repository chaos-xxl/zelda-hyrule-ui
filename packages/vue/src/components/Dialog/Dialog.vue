...
/** 从 Figma 导出的精确对话框背景路径（与 React 版同源） */
...
import styles from '@react/components/Dialog/dialog.module.less'
...
const ariaLabel = computed(() => (props.speaker ? `${props.speaker} dialog` : 'Dialog'))
</script>

<template>
  <div
    :class="[styles.dialog, styles[props.type]]"
    role="region"
    :aria-label="ariaLabel"
  >
    <svg :class="styles.bg" viewBox="0 0 910 185" fill="none" preserveAspectRatio="none" aria-hidden="true">
      <path fill-rule="evenodd" clip-rule="evenodd" :d="DIALOG_BG_PATH" fill="black" fill-opacity="0.5" />
      <path
        v-if="props.type === 'sheikah'"
        :d="DIALOG_OUTLINE_PATH"
        fill="none"
        stroke="rgba(60,211,252,0.6)"
        stroke-width="1.5"
        vector-effect="non-scaling-stroke"
      />
    </svg>
    <div v-if="props.speaker" :class="styles.speaker">{{ props.speaker }}</div>
    <div :class="styles.content"><slot /></div>
    <span v-if="props.showContinue" :class="styles.continueArrow" aria-hidden="true">
      <svg aria-hidden="true" width="40" height="26" viewBox="0 0 38 25.4" fill="none">
        <path d="M19 25.4L0 6.4L6.4 0L19 12.6L31.6 0L38 6.4L19 25.4Z" fill="rgba(226,222,211,0.5)" />
      </svg>
    </span>
  </div>
</template>
