...
| 死磕移动端 Demo 适配 | 🟢 P1 | **游戏主题库最怕之一：移动端稀烂。** 开发者 90% 在手机上刷 GitHub/Twitter/Reddit，Live Demo 第一印象常发生在手机上，排版错乱会立刻划走。BOTW 组件按主机大屏设计（血条/对话框都大），手机上要用等比例缩放（`transform: scale`）显得精致，不能硬塞。**现状：做过一版 + 有 `#/mobile` 页，但体验仍差，需重做。** |
...
| 事项 | 优先级 | 说明 |
|------|--------|------|
| 死磕移动端 Demo 适配 | 🟢 P1 | 见「🔜 计划中」。游戏库怕的"移动端稀烂"，是留存生死线。等比例缩放，手机上也要精致。 |
