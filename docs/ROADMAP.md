...
### S. Showcase 实战案例（面向"游戏玩家 + 大学生"受众）

> 选题标准：**真实用途（非纯炫技）+ BOTW 语义能自然映射（非硬套皮）+ 自带发帖传播点**。都走"路径丙"配方：单文件、零依赖、无构建、真素材、localStorage。每个 = 一条发帖素材 + 一个 skill 活样板。

| 事项 | 优先级 | 说明 |
|------|--------|------|
| June Quest Calendar ✅ | — | 已上线。学生日程表 + 可交互 Quest Log 弹窗。`public/showcase/june-quest-calendar.html` |
| Finals Boss Rush ✅ | — | 已上线。期末复习做成打 Boss（敌人血条/学习扣血/stagger/vanquish/HUD 映射）。`public/showcase/finals-boss-rush.html` |
| 塞尔达风简历 / 个人主页 | 🟢 P1 💡 | 简历做成背包/属性界面：技能=武器栏(熟练度=攻击力)、项目=任务日志、教育=回忆相册、联系方式=地图标记。数据抽成 JSON，别人改 JSON 即生成自己的。**无时效 + 传播上限最高 + 自来水循环（用户作品变宣传素材）。** 1–2 个周末。 |
| 神庙/呀哈哈收集追踪器 | 🟡 P2 💡 | 用户自己勾选的收集进度板（ItemBG 格子矩阵 + 精力轮进度环）。**直击 r/zelda 真玩家，Reddit 发帖最对味。⚠️ 合规：只做空白追踪器，绝不内置神庙名/位置数据库（那是游戏内容数据）。** 1 个周末。 |
| 希卡课程表（周视图） | 🔵 P3 💡 | 周课表，当前课用 Sensor 闪烁定位。**囤到 9 月开学季发帖。** |
| 习惯打卡（Stamina Tracker） | 🔵 P3 💡 | 连续打卡换心心上限（暗合神庙换心机制）+ 月度热力图。常青题材。 |
| 成绩单图鉴 / 倒数日 | 🔵 P3 💡 | 每门课一张 SheikahCompendiumEntry 图鉴卡（GPA=卢比）；或考研/毕业 KPI 大字报倒数日（小红书体质，蹭节点）。 |
